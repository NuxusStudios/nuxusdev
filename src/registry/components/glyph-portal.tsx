...
export default function GlyphPortal({
  ...
  // Adapted from the original's running counter: the React Compiler rejects
  // reassigning a variable during render. Same result — each character paired
  // with its UTF-16 offset, so surrogate pairs still index correctly.
  const glyphs = Array.from(text);
  const characters = glyphs.map((char, i) => ({
    char,
    index: glyphs.slice(0, i).reduce((n, c) => n + c.length, 0),
  }));
  ...
  useLayoutEffect(() => {
    ...
    const layout = () => {
      ...
      if (fontDirty.current) { ready = readInk(); fontDirty.current = false; }
      if (!ready) return;
