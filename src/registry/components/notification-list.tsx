      ...
      <AnimatePresence initial={false}>
        {visible.map((item) => (
          <motion.figure
            ...
            className="flex items-center gap-3 rounded-2xl border border-foreground/10 bg-foreground/[0.04] p-4 backdrop-blur-md"
          >
            <div
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className="shrink-0 text-xs text-foreground/30">{item.time}</span>
              </div>
              <p className="truncate text-sm text-foreground/45">{item.description}</p>
            </div>
          </motion.figure>
        ))}
      </AnimatePresence>
